...
class UpdateForm extends Component{
    ...
    */
    ...
    /*
    각 input상자에서 입력한 내용이 있을때 state값을 변경하기 위해 선언한 함수로,
    setState()함수를 통해 값을 변경한다.
    이벤트 객체를 통해 input상자의 name, value속성을 얻어와서 지정된 값을 변경할 수 있다.
    */
    ...
    render(){
        return(
            <article>
                ...
                >
                    {/* 기존 게시물에 대한 수정임으로 게시물 아이디를 저장할 hidden상자 필요 */}
                    {/*
                        App.js에서 기존 게시물을 props로 전달한 후
                        input상자에 즉시 삽입하면 값을 변경하는 것이 불가능하다.
                        
                        props는 읽기전용 데이터임으로 값의 변경은 state를 통해서만 가능하다.
                        
                        이 경우 props로 전달된 데이터를 통해 state를 초기화하고,
                        각 input상자에서 변경되는 값은 onChange이벤트를 통해 동기화 해야한다.
                    */}
                    <input type="hidden" name="id" value={this.props.readData.id}></input>
