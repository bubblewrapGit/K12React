/*
  react-router-dom
    : 리액트는 기본적으로 화면의 새로고침 없이 화면을 갱신한다.
      ...
      그 상태를 관리하기 위해 내부적으로 state나 props를 관리 할 수 있게 해주는 도구가 
      react-router-dom이다.
*/
...
/* 
BrowserRouter
  : 리액트 라우터 돔을 적용하고 싶은 컴포넌트를 감싸주는 Wrapper(래퍼)로 사용한다.
Route
  : URL에 따른 적당한 컴포넌트를 렌더링하기 위해 사용한다.
Switch
  : URL과 일치하는 첫번째 컴포넌트가 발견되면 나머지 컴포넌트는 아예 렌더링 하지 않는 역할을 한다.
    즉, 최초로 발견되는 컴포넌트 하나만 렌더링 한다.
Link
  : a태그를 통한 링크는 클릭할 때 마다 화면의 새로고침되어 갱신된다.
    React는 화면의 깜빡임이 없어야 함으로, 링크를 눌렀을때 reload되지 않도록 처리해준다.
*/
...
function Topics(){
  ...
  for(var i=0; i<contents.length; i++){
    ...
    /* 
    li태그와 같이 반복되는 엘리먼트를 사용하는 경우 React는 중복되지 않는 Key prop을 요구한다.
    ...
    <BrowserRouter>
      <div className="App">
        ...
        {/* 
          ...
          exact
            : <Route 컴포넌트에 삽입하면 path(경로)가 정확히 잂치하는 경우에만 라우팅된다.
        ...
        */}
        ...
        <h3>Switch적용</h3>
        {/* 
          Switch 컴포넌트를 사용하는 경우 첫번째 Route에 exact를 기술하지 않으면
           / 가 모든 URL을 포함하는 형태임으로 아래쪽에 정의된 컴포넌트는 렌더링 되지 않는다.
